<template>
	<v-container fluid>
		...
		<radar-chart v-if="this.chartReady" :chartData="this.chartData" :key="renderKey" />
		...
		<v-row>
			...
			>Add another</v-btn>-->
			<v-spacer />
			<v-btn color="primary" class="mr-4" :disabled="loading" @click="this.createPlaylistFromSelection">Done</v-btn>
...
export default {
	...
	methods: {
		createPlaylistFromSelection() {
			let name = this.playlistName != undefined ? this.playlistName : this.defaultPlaylistName;
	...
	mounted: function() {
		...
		this.audioFeatures = _.zipWith(this.playlistDetailsTemp, this.audioFeaturesTemp, function(a, b) {
