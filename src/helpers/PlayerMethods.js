export function getCurrentTrack() {
	...
	this.$http
		.get(`http://https://d2ob92q3jfbd5e.cloudfront.net/player?q=current&credentials=${localStorage.RunBPM}`)
		.then((response) => {
...
export function getTrackDetails(id) {
	this.$http.get(`http://https://d2ob92q3jfbd5e.cloudfront.net/analyze-tracks?id=${id}&credentials=${localStorage.RunBPM}`).then((response) => {
...
export function updatePlayState(event) {
	...
	const play = () => {
		this.$http
			.put(`http://https://d2ob92q3jfbd5e.cloudfront.net/player?action=play&credentials=${localStorage.RunBPM}`, {
				data: {
					...options,
				},
			})
	...
	const pause = () => {
		this.$http
			.put(`http://https://d2ob92q3jfbd5e.cloudfront.net/player?action=pause&credentials=${localStorage.RunBPM}`)
			.then(() => this.getCurrentTrack())
			.catch((error) => console.log(error));
	};
	const previous = () => {
		this.$http
			.put(`http://https://d2ob92q3jfbd5e.cloudfront.net/player?action=previous&credentials=${localStorage.RunBPM}`, {
				data: this.options,
			})
			.then(() => this.getCurrentTrack())
			.catch((error) => console.log(error));
	...
	const next = () => {
		this.$http
			.put(`http://https://d2ob92q3jfbd5e.cloudfront.net/player?action=next&credentials=${localStorage.RunBPM}`)
			.then(() => this.getCurrentTrack())
			.catch((error) => console.log(error));
	};
	switch (event.state) {
